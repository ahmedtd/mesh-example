...
var map_PKCS10RequestParameterSpec = map[string]string{
	"":           "PKCS10RequestParameterSpec allows the pod author to minimally customize the stub PKCS#10 certificate signing request that Kubelet will generate using the subject private key.\n\nSee the ACME Identifier Types registry [1] for PKCS#10 fields that ACME-compatible CAs *may* support.\n\nIn total, up to 100 SANs may be requested.\n\n[1] https://www.iana.org/assignments/acme/acme.xhtml#acme-identifier-types",
	"dns":        "DNS Subject Alternate Names that will be included in the request.\n\nEach entry will be validated as a DNS name (maximum length 253, all ASCII characters).",
	"ipAdresses": "IP Address Subject Alternate Names that will be included in the request.\n\nEntries must be either IPv4 addresses in dotted-decimal form (\"192.0.2.1\"), or IPv6 addresses (\"2001:db8::68\").",
}

func (PKCS10RequestParameterSpec) SwaggerDoc() map[string]string {
	return map_PKCS10RequestParameterSpec
}
...
var map_PodCertificateProjection = map[string]string{
	...
	"pkcs10RequestParameters": "Requested customizations for the stub PKCS#10 certificate signing request that Kubelet will generate using the subject private key.\n\nCertain CA implementations (for example, those based on the ACME specification), require a PKCS#10 certificate signing request with specific values in order to issue a certificate.\n\nNote that this field is only *advisory* for the signer implementation and its backing CA.  For example, if you request a DNS SubjectAlternateName that the CA does not believe is valid for your pod, it will reject your certificate requests.",
}
