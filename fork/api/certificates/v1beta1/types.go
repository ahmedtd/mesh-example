...
type PodCertificateRequestSpec struct {
	...
	// UnverifiedPKCS10Request is set, this field must be empty.  Signer
	// implementations should extract the public key from the
	// UnverifiedPKCS10Request field.
	//
	// +optional
	PKIXPublicKey []byte `json:"pkixPublicKey" protobuf:"bytes,9,opt,name=pkixPublicKey"`

	// A proof that the requesting kubelet holds the private
	...
	// request one or more SANs using the pkcs10RequestParameters field on the
	// pod certificate projected volume spec.
	//
	// The signer and CA implementation MUST NOT blindly issue certificates that
	// contain the SANs that the pod spec author requested.  For example, if the
	// PKCS#10 request contains a DNS subject alternate name, this only
	// indicates that the pod spec author *requested* this DNS name.  The signer
	// and/or backing CA MUST verify that the pod is entitled to use that DNS
	// name (for example, using ACME challenges).
	UnverifiedPKCS10Request []byte `json:"unverifiedPKCS10Request" protobuf:"bytes,12,opt,name=unverifiedPKCS10Request"`
