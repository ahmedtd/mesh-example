...
var map_PodCertificateRequestSpec = map[string]string{
	...
	"pkixPublicKey":             "The PKIX-serialized public key the signer will issue the certificate to.\n\nThe key must be one of RSA3072, RSA4096, ECDSAP256, ECDSAP384, ECDSAP521, or ED25519. Note that this list may be expanded in the future.\n\nSigner implementations do not need to support all key types supported by kube-apiserver and kubelet.  If a signer does not support the key type used for a given PodCertificateRequest, it must deny the request by setting a status.conditions entry with a type of \"Denied\" and a reason of \"UnsupportedKeyType\". It may also suggest a key type that it does support in the message field.\n\nDEPRECATED: This field is replaced by UnverifiedPKCS10Request. If UnverifiedPKCS10Request is set, this field must be empty.  Signer implementations should extract the public key from the UnverifiedPKCS10Request field.",
	"proofOfPossession":         "A proof that the requesting kubelet holds the private key corresponding to pkixPublicKey.\n\nIt is contructed by signing the ASCII bytes of the pod's UID using `pkixPublicKey`.\n\nkube-apiserver validates the proof of possession during creation of the PodCertificateRequest.\n\nIf the key is an RSA key, then the signature is over the ASCII bytes of the pod UID, using RSASSA-PSS from RFC 8017 (as implemented by the golang function crypto/rsa.SignPSS with nil options).\n\nIf the key is an ECDSA key, then the signature is as described by [SEC 1, Version 2.0](https://www.secg.org/sec1-v2.pdf) (as implemented by the golang library function crypto/ecdsa.SignASN1)\n\nIf the key is an ED25519 key, the the signature is as described by the [ED25519 Specification](https://ed25519.cr.yp.to/) (as implemented by the golang library crypto/ed25519.Sign).\n\nDEPRECATED: This field is replaced by UnverifiedPKCS10Request. If UnverifiedPKCS10Request is set, this field must be empty.",
	"unverifiedPKCS10Request":   "A PKCS#10 certificate signing request (DER-serialized) generated by Kubelet using the subject private key.\n\nMost signer implementations will ignore the contents of the CSR except to extract the subject public key. The API server automatically verifies the CSR signature during admission, so the signer does not need to repeat the verification.\n\nThe subject public key must be one of RSA3072, RSA4096, ECDSAP256, ECDSAP384, ECDSAP521, or ED25519. Note that this list may be expanded in the future.\n\nSigner implementations do not need to support all key types supported by kube-apiserver and kubelet.  If a signer does not support the key type used for a given PodCertificateRequest, it must deny the request by setting a status.conditions entry with a type of \"Denied\" and a reason of \"UnsupportedKeyType\". It may also suggest a key type that it does support in the message field.\n\nSome CA implementations (for example, those using the ACME protocol) require that the client (the signer implementation, in this case) provide a PKCS#10 certificate signing request, even if the CA only extracts the subject public key from the request.  To enable compatibility with these CAs, Kubelet will generate a stub PKCS#10 request that the signer implementation can then pass on to the CA.\n\nBy default the request is completely empty, but the pod spec author may request one or more SANs using the pkcs10RequestParameters field on the pod certificate projected volume spec.\n\nThe signer and CA implementation MUST NOT blindly issue certificates that contain the SANs that the pod spec author requested.  For example, if the PKCS#10 request contains a DNS subject alternate name, this only indicates that the pod spec author *requested* this DNS name.  The signer and/or backing CA MUST verify that the pod is entitled to use that DNS name (for example, using ACME challenges).",
