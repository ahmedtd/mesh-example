...
type PodCertificateRequestSpecApplyConfiguration struct {
	...
	UnverifiedPKCS10Request []byte `json:"unverifiedPKCS10Request,omitempty"`
...
// WithUnverifiedPKCS10Request adds the given value to the UnverifiedPKCS10Request field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the UnverifiedPKCS10Request field.
func (b *PodCertificateRequestSpecApplyConfiguration) WithUnverifiedPKCS10Request(values ...byte) *PodCertificateRequestSpecApplyConfiguration {
	for i := range values {
		b.UnverifiedPKCS10Request = append(b.UnverifiedPKCS10Request, values[i])
	}
	return b
}
